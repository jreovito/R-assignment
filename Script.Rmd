...
#### create a for-loop that cycles through chromosomes 1-10 in the joined_maize data file in my global env and creates txt files in increasing order

```{r}
for (i in 1:10) {inc_maize <- subset(joined_maize, joined_maize[[3]] == i)  # Filters rows where the third column matches the chromosome number
inc_maize[is.na(inc_maize)] <- "?" # Replaces NA values with "?"
inc_maize <- inc_maize[order(inc_maize[[2]]), ]  # Sorts the data by the second column in increasing order
...
```{r}
for (i in 1:10) {inc_teosinte <- subset(joined_teosinte, joined_teosinte[[3]] == i) # Creates a new variable name for each chromosome dataset
inc_teosinte[is.na(inc_teosinte)] <- "?"  # Replaces NA values with "?"
inc_teosinte <- inc_teosinte[order(inc_teosinte[[2]]), ]  # Sorts the data by the second column in increasing order
...
```{r}
for (i in 1:10) {
  dec_maize <- subset(joined_maize, joined_maize[[3]] == i) # Creates a new variable for each chromosome dataset
  dec_maize[dec_maize == "?"] <- "-" # Replaces "?" with "-"
  dec_maize <- dec_maize[order(dec_maize[[2]], decreasing = TRUE), ] # Sorts by the second column in decreasing order
...
#### From what I can tell based on the graph it seems as though teosinte and maize have the same number of SNPs in each chromosome, although I am not entirely sure if this is correct given evolutionary relationships between the 2 species.

#### Heterozygosity and missing Data visualization
...
#### I am not entirely sure if this is exactly what the question was asking but I tried my best to create the graph that I thought represented what the question wanted

#### Data Visualization Part III: My own visualization: Which species has the smallest SNP length?

#### this calculates the minimum SNP length in both maize and teosinte
...
#### plotting the minimum SNP length
...
#### as you can see both maize and teosinte have the same minimum SNP length, again, given the evolutionary relationship I am not entirely convinced this is correct but I couldn't figure out how to fix this issue of them having the same amount of SNPs
